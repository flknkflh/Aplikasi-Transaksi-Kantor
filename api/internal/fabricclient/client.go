// Package fabricclient wraps the Fabric Gateway Go SDK connection used to
// submit chaincode transactions. The identity/TLS material here is the
// application's Fabric MSP identity (its "which organization is this
// backend acting as" credential) — a separate concept from the per-user
// classical+PQC business signatures in ledger/crypto, which authenticate
// *who within the organization* asked for the action. See
// docs/Hybrid_PQC_Permissioned_Blockchain_PRD.md §6 for that distinction.
...
func Connect(cfg config.Config) (*Client, error) {
	...
	return &Client{
		conn:    conn,
		gateway: gw,
		network: gw.GetNetwork(cfg.FabricChannelName),
	}, nil
}

func (c *Client) Contract(chaincodeName string) *client.Contract {
...
// readFirstFile matches the fabric-gateway sample apps' convention: the
// MSP cert/key directories contain exactly one file each, named by hash
