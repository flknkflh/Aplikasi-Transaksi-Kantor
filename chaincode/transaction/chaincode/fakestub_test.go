...
// It embeds the (nil) interface so it satisfies the full, large
// ChaincodeStubInterface at compile time; anything not overridden below
// would panic on a nil-pointer call if ever invoked, which is fine since the
// contract under test doesn't use those methods.
