// Package fabricclient wraps the Fabric Gateway connection the audit-service
// uses to query the ledger directly (EvaluateTransaction only — the
// audit-service never submits/writes). Querying the ledger itself, not
// Postgres, is what makes its verification independent of the API/indexer
// (PRD FR-010). See api/internal/fabricclient for the identity/TLS material
// explanation.
...
func (c *Client) Network() *client.Network {
	return c.network
}
